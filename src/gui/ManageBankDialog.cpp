...
void udManageBankDialog::OnRemoveCat(wxCommandEvent& event)
{
	...
	if( pCat )
	{
		...
		if( pCat->HasChildren() )
		{
			if( wxMessageBox( _("Selected category is not empty. Are you sure you want to remove it?"), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION ) == wxNO ) return;
		}
...
{
	if( wxMessageBox( _("Are you sure you want to remove selected diagram?"), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION ) == wxYES )
	{
...
{
	if( wxMessageBox( _("Are you sure you want to remove all diagrams from this category?"), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION ) == wxYES )
	{
		udDiagramBank& Bank = wxGetApp().GetDiagramBank();
		udDiagramCategory *pCat = Bank.GetCategory( m_lbCategories->GetStringSelection() );
		if( pCat )
...
udDiagramRecord* udManageBankDialog::CopyRecord()
{
	...
	dlg.SetTitle( _("Move diagram") );
	dlg.SetComment( _("Select target category:") );
