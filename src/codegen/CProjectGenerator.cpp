...
void udCProjectGenerator::ProcessProject(udProject *src)
{
	...
	{
		wxMessageBox(_("Output directory '") + sPath + _("' doesn't exist. Please check the project settings."), wxT("CodeDesigner"), wxICON_ERROR | wxOK );
		return;
	}
	...
	if( m_pOutLang )
	{
		...
		while( node )
		{
			...
			progressDlg.SetLabel(  wxString::Format( _("Generating '%s'..."), pDiagram->GetName().c_str() ) );
			Log( wxString::Format( _("Generating code for diagram '%s'..."), pDiagram->GetName().c_str() ) );
			...
			Log( wxString::Format( _("Output files: %s, %s."), OutFile.GetFullPath().c_str(), HeaderFile.GetFullPath().c_str() ) );
			...
			if( pGenerator )
			{
				...
				if( !fSuccess ) wxMessageBox(_("Generation process finished with ERROR status. See the log window for more details."), wxT("CodeDesigner"), wxOK | wxICON_WARNING);
