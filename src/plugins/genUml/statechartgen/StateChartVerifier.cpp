...
bool udStateChartVerifier::CheckInitialStates(udDiagramItem *diagram)
{
	...
	if( !node )
	{
		IPluginManager::Get()->Log(wxString::Format(_("ERROR: there is no initial state in diagram '%s'."), diagram->GetName().c_str()));
		fSuccess = false;
	}
	while(node)
	{
		...
		if( lstParents.IndexOf(pShape->GetParentShape()) != wxNOT_FOUND )
		{
			...
			IPluginManager::Get()->Log(wxString::Format(_("ERROR: more than one initial state in a parent object (state: '%s')"), pElement->GetName().c_str()));
...
bool udStateChartVerifier::CheckEndingStates(udDiagramItem *diagram)
{
	...
	{
		IPluginManager::Get()->Log(wxString::Format(_("WARNING: there should be at least one ending state in diagram '%s'."), diagram->GetName().c_str()));
	}
...
bool udStateChartVerifier::CheckUnconnectedStates(udDiagramItem *diagram)
{
	...
	while(node)
	{
		...
		if( lstOuttrans.IsEmpty() )
		{
			...
			IPluginManager::Get()->Log(wxString::Format(_("ERROR: there is unconnected state '%s' in diagram '%s'."), pElement->GetName().c_str(), diagram->GetName().c_str()));
		}
...
bool udStateChartVerifier::_CheckOutcommingPaths(wxSFDiagramManager *manager, wxSFShapeBase *parent)
{
	...
	while(node)
	{
		...
		else
		{
			...
			{
				IPluginManager::Get()->Log(wxString::Format(_("ERROR: there are tansitions with duplicated guard condition in state '%s'."), pStateElement->GetName().c_str()));
				fSuccess = false;
			}
			...
			{
				IPluginManager::Get()->Log(wxString::Format(_("ERROR: there are tansitions with duplicated priority level in state '%s'."), pStateElement->GetName().c_str()));
				fSuccess = false;
			}
		}
	...
	if( nConditionLess > 1 )
	{
		IPluginManager::Get()->Log(wxString::Format( _("ERROR: there are more than one conditionless transitions in state '%s'."), pStateElement->GetName().c_str()));
		fSuccess = false;
	}
...
bool udStateChartVerifier::CheckHistoryStates(udDiagramItem *diagram)
{
	...
	while( node )
	{
		...
		{
			IPluginManager::Get()->Log( wxString::Format(_("ERROR: history state '%s' has no parent."), udLABEL::GetContent( pHistory, udLABEL::ltTITLE ).c_str() ) );
			fSuccess = false;
		}
		...
		if( !fPartSuccess )
		{
			IPluginManager::Get()->Log( wxString::Format( _("ERROR: there must by exactly one conditionless incoming transition in history state '%s' which starts either in an initial state or in parent composite state."), udLABEL::GetContent( pHistory, udLABEL::ltTITLE ).c_str() ) );
			fSuccess = false;
		}
		...
		{
			IPluginManager::Get()->Log( wxString::Format( _("ERROR: there must by exactly one conditionless outcoming transition in history state '%s'."), udLABEL::GetContent( pHistory, udLABEL::ltTITLE ).c_str() ) );
			fSuccess = false;
		}
...
bool udStateChartVerifier::CheckGOTOConstraints(udDiagramItem* diagram)
{
	...
	if( pSSCh && (pSSCh->GetActiveAlgorithm() == wxT("udGOTOAlgorithm")) )
	{
		...
		{
			IPluginManager::Get()->Log( wxString::Format( _("ERROR: 'GOTO' code generation algorithm set in diagram '%s' doesn't support input actions."), diagram->GetName().c_str() ) );
			return false;
		}
		if( pSSCh->IsNonBlocking() )
		{
			IPluginManager::Get()->Log( wxString::Format( _("ERROR: 'GOTO' code generation algorithm set in diagram '%s' doesn't support non-blocking state charts."), diagram->GetName().c_str() ) );
			return false;
		}
	}
	return true;
