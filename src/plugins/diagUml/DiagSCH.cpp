...
bool udTransElementItem::SetGuardString(const wxString &guard)
{
	...
	{
		wxMessageBox( wxString::Format( _("Given guard content doesn't match the allowed form '%s'."), sRegex.c_str() ), wxT("CodeDesigner"), wxICON_WARNING | wxOK);
	}
...
void udTransElementItem::SetConditionString(const wxString& cond)
{
	...
	if( reGuard.Matches( cond ) )
	{
		...
		if( !sCond.IsEmpty() )
		{
			...
			{
				if( wxMessageBox( wxString::Format( _("Condition with name '%s' doesn't exist yet. Would you like to create one?"), sCond.c_str() ), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION  ) == wxYES )
				{
		 ...
		 wxMessageBox( wxString::Format( _("Name '%s' doesn't match the allowed form '%s'."), cond.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
}
...
void udTransElementItem::SetActionString(const wxString& acts)
{
	...
	if( reGuard.Matches( acts ) )
	{
		...
		while ( tokens.HasMoreTokens() )
		{
			...
			{
				if( wxMessageBox( wxString::Format( _("Action with name '%s' doesn't exist yet. Would you like to create one?"), sAct.c_str() ), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION  ) == wxYES )
				{
					udProjectItem *pRoot =  (udProjectItem*) IPluginManager::Get()->GetProject()->GetRootItem();
					// create new condition
		...
		RestoreCallParams();
		 ...
		 wxMessageBox( wxString::Format( _("Action list '%s' doesn't match the allowed form '%s'."), acts.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
}
...
wxMenu* udTransElementItem::CreateMenu()
{
	...
	wxMenuItem *pItem = new wxMenuItem(pCreateSubMenu, IDM_TRANS_CREATECONDITION, _("Condition function"));
	pItem->SetBitmap(IPluginManager::Get()->GetArtBitmap(wxT("udConditionItem")));
	pCreateSubMenu->Append(pItem);

	pItem = new wxMenuItem(pCreateSubMenu, IDM_TRANS_CREATEEVENT, _("Event flag"));
	pItem->SetBitmap(IPluginManager::Get()->GetArtBitmap(wxT("udEventItem")));
	pCreateSubMenu->Append(pItem);

	pItem = new wxMenuItem(pCreateSubMenu, IDM_TRANS_CREATEACTION, _("Action function"));
	...
	if( !lstItems.IsEmpty() ) pPopupMenu->Append(wxID_ANY, _("Assign condition/event"), pAssignSubMenu);
	...
	if( !lstItems.IsEmpty() ) pPopupMenu->Append(wxID_ANY, _("Assign action function"), pAssignSubMenu);
	...
	pPopupMenu->Append(IDM_DIAG_CLEARCONDITIONS, _("Clear condition/event"));
	pPopupMenu->Append(IDM_DIAG_CLEARACTIONS, _("Clear action functions"));
...
void udCompStateElementItem::SetActionsString(const wxString& txt, udStateActionLinkItem::TYPE type)
{
	...
	if( reGuard.Matches( txt ) )
	{
		...
		while ( atokens.HasMoreTokens() )
		{
			...
			{
				if( wxMessageBox( wxString::Format( _("Action with name '%s' doesn't exist yet. Would you like to create it?"), sAct.c_str() ), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION  ) == wxYES )
				{
					udProjectItem *pRoot =  (udProjectItem*) IPluginManager::Get()->GetProject()->GetRootItem();
					// create new action
					udActionItem* pNewAct = (udActionItem*) IPluginManager::Get()->GetProject()->CreateProjectItem( wxT("udActionItem"), pRoot->GetId() );
		...
		wxMessageBox( wxString::Format( _("State action '%s' doesn't match the allowed form '%s'."), txt.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
}
...
wxMenu* udCompStateElementItem::CreateMenu()
{
	...
	wxMenuItem *pItem = new wxMenuItem(pCreateSubMenu, IDM_COMPSTATE_ACTION, _("Action function"));
	pItem->SetBitmap(IPluginManager::Get()->GetArtBitmap(wxT("udActionItem")));
	pCreateSubMenu->Append(pItem);
