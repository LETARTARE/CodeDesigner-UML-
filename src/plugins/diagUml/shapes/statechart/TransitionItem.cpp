...
void umlTransitionItem::OnChildDropped(const wxRealPoint& pos, wxSFShapeBase* child)
{
	...
	{
		wxMessageBox( wxString::Format( _("'%s' is not accepted by '%s'"), pOriginal->GetName().c_str(), pTransElement->GetName().c_str() ), wxT("CodeDesigner"), wxICON_WARNING | wxOK );
		return;
	}
			 ...
			 pLink->IsKindOf( CLASSINFO(udDiagramLinkItem) ) )
	{
		...
		if(pFinalElement)
		{
			...
			{
				wxMessageBox( _("Only final states from source sub machine can be dropped here."), wxT("CodeDesigner"), wxICON_WARNING | wxOK );
				return;
			}
		...
		if( pClassElement )
		{
			...
			if( !pLang->HasClasses() )
			{
				wxMessageBox( _("Selected language doesn't support class instantiation."), wxT("CodeDesigner"), wxOK | wxICON_ERROR );
				return;
			}
