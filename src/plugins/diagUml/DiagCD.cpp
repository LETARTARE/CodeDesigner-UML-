	...
	if( pFcnLink )
	{
		...
		else
		{
			...
			if( reGuard.Matches( txt ) )
			{
				...
				{
					if( wxMessageBox( wxString::Format( _("Class function member with name '%s' doesn't exist in this scope yet. Would you like to create one?"), sFunction.c_str() ), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION  ) == wxYES )
					{
				...
				wxMessageBox( wxString::Format( _("Function '%s' doesn't match the allowed form '%s'."), txt.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
		}
	}
}
	...
	if( pVarLink )
	{
		...
		else
		{
			...
			if( reGuard.Matches( txt ) )
			{
				...
				{
					if( wxMessageBox( wxString::Format( _("Class data member with name '%s' doesn't exist in this scope yet. Would you like to create one?"), sVariable.c_str() ), wxT("CodeDesigner"), wxYES_NO | wxICON_QUESTION  ) == wxYES )
					{
						udProjectItem *pRoot =  (udProjectItem*) IPluginManager::Get()->GetProject()->GetRootItem();
				...
				wxMessageBox( wxString::Format( _("Variable '%s' doesn't match the allowed form '%s'."), txt.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
		}
	}
}
...
wxMenu* udClassElementItem::CreateMenu()
{
	...
	pCreateSubMenu->Append( IDM_CLASS_CREATECONSTRUCTOR, _("Create constructor") );
	pCreateSubMenu->Append( IDM_CLASS_CREATEDESTRUCTOR, _("Create destructor") );

	pCreateSubMenu->AppendSeparator();

	wxMenuItem *pItem = new wxMenuItem(pCreateSubMenu, IDM_CLASS_CREATEVARIABLE, _("Member variable"));
	pItem->SetBitmap(IPluginManager::Get()->GetArtBitmap(wxT("udMemberDataItem")));
	pCreateSubMenu->Append(pItem);

	pItem = new wxMenuItem(pCreateSubMenu, IDM_CLASS_CREATEFUNCTION, _("Member function"));
	pItem->SetBitmap(IPluginManager::Get()->GetArtBitmap(wxT("udMemberFunctionItem")));
	pCreateSubMenu->Append(pItem);

    pPopupMenu->Append(wxID_ANY, _("Create"), pCreateSubMenu);
	...
	if( nId ) pPopupMenu->Append(wxID_ANY, _("Assign member variable"), pAssignSubMenu);
	...
	if( nId ) pPopupMenu->Append(wxID_ANY, _("Assign member function"), pAssignSubMenu);
	...
	if( HasChildren() )
	{
		...
		pPopupMenu->Append(wxID_ANY, _("Edit"), pEditSubMenu);
		pPopupMenu->AppendSeparator();
	}

	pPopupMenu->Append(IDM_DIAG_CLEARVARIABLES, _("Clear member variables"));
	pPopupMenu->Append(IDM_DIAG_CLEARFUNCTIONS, _("Clear member functions"));

	pPopupMenu->AppendSeparator();

	// create popup menu
	pItem = new wxMenuItem(pPopupMenu, wxID_COPY, _("Copy\tCtrl+C"));
...
void udClassElementItem::OnTreeItemEndDrag(const wxPoint &pos)
{
	...
	if( !pPresentItem || (pPresentItem == pSourceItem) )
	{
		...
		{
			wxMessageBox( _("The code link cannot be used in this scope."), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
		}
		else
...
void udTemplateBindElementItem::SetBindTypeString(const wxString& txt)
{
	...
	else
	{
		...
		wxMessageBox( wxString::Format( _("Bind type '%s' doesn't match the allowed form '%s'."), txt.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
	}
}
...
wxMenu* udEnumElementItem::CreateMenu()
{
	...
	pMenu->Insert(nIndex++, IDM_ENUM_ADDELEMENT, _("Add element"));
	...
	if( HasChildren() )
	{
		...
		pMenu->Insert(nIndex++, wxID_ANY, _("Edit"), pEditSubMenu);
	}

	pMenu->InsertSeparator(nIndex++);
	pMenu->Insert(nIndex++, IDM_ENUM_CLEARELEMENTS, _("Clear all elements"));
	pMenu->InsertSeparator(nIndex++);
...
bool udEnumElementItem::CheckElementString(const wxString& element, wxString& key, wxString& val)
{
	...
	{
		wxMessageBox( wxString::Format( _("Element '%s' doesn't match the allowed form '%s'."), element.c_str(), sRegex.c_str() ), wxT("CodeDesigner"), wxOK | wxICON_WARNING );
		return false;
	}
}
...
wxMenu* udIncludeAssocElementItem::CreateMenu()
{
	...
	pMenu->Insert( 0, wxID_ANY, _("Access"), CreateAccessMenu() );
...
wxMenu* udMemberFunctionItem::CreateMenu()
{
	...
	{
		pPopupMenu->Insert(0, IDM_CLASS_RENAMEVIRTUAL, _("Refactor virtual function"), _("Rename all correspoding ancestors/successors."));
