...
{
	wxMessageBox( _("Test action"), wxT("CodeDesigner") );
}
...
void udTestAlgorithm::ProcessAlgorithm(udDiagramItem* src)
{
	...
	for( ShapeList::iterator it = lstElements.begin(); it != lstElements.end(); ++it )
	{
		...
		else
		{
			...
			IPluginManager::Get()->Log( wxString::Format(_("WARNING: '%s' element is not supported by this algorithm."), pElement->GetName().c_str()) );
		}
	}
}
